...
Tests failed: `r fail`
<br>
`r if(advisory>0) {paste("Recommendations: ",advisory," - please check the tests for recommended changes to your files.")}`
...
**`r if(data_advisory>0) {paste("Please check the recommended changes to your data file.")}`**
 ...
 **Checking that LA or Regional data are not mislabelled** `r pass_fail_image(incorrect_level_result)`
```{r incorrect_level,echo=FALSE,warning=FALSE,comment=NA}
incorrect_level(dataset)
```
...
**`r if(meta_advisory>0) {paste("Please check the recommended changes to your metadata file.")}`**
