...
<br>
```{r files}
dataset <- read.csv(file_data,stringsAsFactors = FALSE,encoding = "UTF-16")
metadata <- read.csv(meta_data,stringsAsFactors = FALSE,encoding = "UTF-16")
```
...
```{r pre_function, include=FALSE}
  indicators_u <- filter(metadata,col_type == "Indicator")
...
cat(indicator_unit_validation(metadata))
```
